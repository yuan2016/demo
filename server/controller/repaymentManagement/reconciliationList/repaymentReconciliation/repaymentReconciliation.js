...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let timeOption = 't.repayment_time'
    if( params.endTime !== '') {
    ...
    let query = sql.repaymentManagement.repaymentReconciliation.selectAllFront + queries + timeLimit + add + order + sql.repaymentManagement.repaymentReconciliation.selectAllBack
  ...
  getCount (req, res) {
    ...
    let add = mosaic(params, 'user_phone', 't1')
    let query = sql.repaymentManagement.repaymentReconciliation.getCount + queries + timeLimit + add
