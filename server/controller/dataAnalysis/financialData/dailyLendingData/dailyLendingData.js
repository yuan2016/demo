...
let {formatCurrency, formatInt, analysis, mosaicName, handleProperty, handleTime, combine} = require('../../../../utils/utils')
...
function formatExcelData (rows) {
  return rows.map(row => {
    ...
    if (row['7天期限放款总额']) {
      row['7天期限放款总额'] = formatCurrency(row['7天期限放款总额'])
    }
    if (row['14天期限放款总额']) {
      row['14天期限放款总额'] = formatCurrency(row['14天期限放款总额'])
    }
    if (row['21天期限放款总额']) {
      row['21天期限放款总额'] = formatCurrency(row['21天期限放款总额'])
    }
    if (row['21天分期放款总额']) {
      row['21天分期放款总额'] = formatCurrency(row['21天分期放款总额'])
    }
    if (row['21天分期提额放款金额']) {
      row['21天分期提额放款金额'] = formatCurrency(row['21天分期提额放款金额'])
    }
    if (row['90天分期放款金额']) {
      row['90天分期放款金额'] = formatCurrency(row['90天分期放款金额'])
    }
    if (row.老用户放款总额) {
      row.老用户放款总额 = formatCurrency(row.老用户放款总额)
    }
    if (row.新用户放款总额) {
      row.新用户放款总额 = formatCurrency(row.新用户放款总额)
    }
    //num
    if (row.注册人数) {
...
module.exports = {
  ...
  getExcelData (req, res) {
    ...
    let query = sql.dataAnalysis.dailyLendingDataExcel + combined + order
    func.connPool1(query, [tableName.dailyLendingData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
